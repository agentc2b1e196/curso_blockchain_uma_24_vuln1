...
contract FP_DAO is IFP_DAO, AccessControl {
    ...
    IERC20 public immutable fptContract; //Voting power
    ...
    */
    ...
    function endDispute(uint256 disputeId) external {  
        if (disputes[disputeId].totalVoters < quorum) { //Logica. calcVotingPower
        ...
        if (disputes[disputeId].votesFor > disputes[disputeId].votesAgainst) {
            ...
            _buyerWins(itemId); //Reentrancy? Patrón CEI. Logica
            disputeResult[disputeId] = Vote.FOR;
        } else {
            delete disputes[disputeId];
            _sellerWins(itemId); //Reentrancy? Patrón CEI. Logica
    ...
    function _lotteryNFT(address user) internal { 
        ...
        emit AwardNFT(user); //fallo de lógica, dependiendo de cómo esté implementado el programa que recoja el evento
    }
